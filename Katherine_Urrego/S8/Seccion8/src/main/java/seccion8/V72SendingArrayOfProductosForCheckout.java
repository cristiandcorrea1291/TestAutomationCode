...
public class V72SendingArrayOfProductosForCheckout {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for(int i=0;i<products.size();i++) { 
			...
			if (name.contains("Cucumber")){ // Sí la lista continene el formatedName del arreglo de elementos name
		...
		for(int i=0;i<products.size();i++) { 		
			...
			if (name.contains("Brocolli")){ // Sí la lista continene el formatedName del arreglo de elementos name
			// click en el botón "Add to cart" que hay en cada producto
			driver.findElements(By.xpath("//div[@class='product-action']/button")).get(i).click();
			break; //para salir del loop una vez cumpla la condicion del for o el statement
